...
from multi.data import get_dataloader, MultiTransactionDataset, get_tokenizer_cached
...
class TuningManager:
    ...
    def __init__(self, args, study_name):
        ...
        if self.args.unfreeze_last_n_layers is None or self.args.unfreeze_last_n_layers > 0:
            self.embedding_service = None
            token_config.use_cached_embeddings = False
        else:
            self.embedding_service = EmbeddingService.create(EmbeddingService.Params(model_name=EmbModel.MPNET))
            token_config.use_cached_embeddings = True
    ...
    def objective(self, trial):
        ...
        norm_type = "layer_norm" # trial.suggest_categorical("normalization_type", ["layer_norm", "rms_norm"])
        dropout = trial.suggest_float("dropout", 0.3, 0.45)
        num_layers = trial.suggest_int("num_layers", 6, 7)
        num_heads = 8 #trial.suggest_categorical("num_heads", [4, 8])
        hidden_dim = trial.suggest_categorical("hidden_dim", [512])
        contrastive_loss_weight = trial.suggest_float("contrastive_loss_weight", 0.2, 0.25)
